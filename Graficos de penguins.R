...
#1.- Convertir las variables categóticas o factores
...
# 1.- Creación de un vector de color
color=c("darkseagreen2", "lavender", "thistle")

# 2.- Creación del gráfico
GB1<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "deeppink3", fill=color)+
...
# 4.- Barras verticales
color=c("violet", "violetred", "thistle1")

GB2<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "royalblue2", fill=color)+
  ggtitle("Gráfico de Barras")+
  xlab("Año")+
  ylab("Frecuencias")+
  coord_flip()+
...
HG<-ggplot(penguins, aes(x=largo_aleta_mm))+
  geom_histogram(col="slategray1", fill="steelblue2")+
  ggtitle("Histograma")+
...
GD<-ggplot(penguins, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  geom_point(aes(color=especie))+
  scale_color_manual(values=c("purple3","olivedrab2","hotpink"))+
  ggtitle("Gráfico de dispersión")+
  xlab("largo del pico (mm)")+
  ylab("grosor del pico (mm)")+
  theme_light()
#scale color manual pa los colores
